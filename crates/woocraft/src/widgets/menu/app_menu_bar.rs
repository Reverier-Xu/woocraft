...
impl AppMenu {
  pub(super) fn new(
    ix: usize, menu: &OwnedMenu, menu_bar: Entity<AppMenuBar>, cx: &mut App,
  ) -> Entity<Self> {
    ...
    let popup_menu = match self.popup_menu.as_ref() {
      None => {
        ...
        let popup_menu = PopupMenu::build(window, cx, |menu, window, cx| {
          menu
            .when_some(window.focused(cx), |this, handle| {
  ...
  fn handle_dismiss(
    &mut self, _: &Entity<PopupMenu>, _: &DismissEvent, window: &mut Window, cx: &mut Context<Self>,
  ) {

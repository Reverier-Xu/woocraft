...
- 弹层布局契约：主垂直堆栈零 padding 零 gap；title / actions 区块各持
  `0.25rem` padding 与 gap；纯文本 content 区块持 `0.5rem` padding；
  title 走 IconLabel（默认图标 + 省略号），actions 右对齐；title 与
  content 之间必有 hairline 分割

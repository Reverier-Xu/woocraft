//! internationalization with a woocraft-owned translation domain.
//!
//! built-in translations live in `locales/*.toml` under the
//! [`WOOCRAFT_I18N_DOMAIN`] prefix and are compiled in via `rust-i18n`.
//! applications can layer their own translations on top: [`load_locale`] and
//! [`extend_locale`] register custom tables that take priority over built-ins
//! while merging with them, so incomplete custom tables never hide built-in
//! strings. keys inside the woocraft domain are protected from overriding by
//! custom tables.
...
/// Qualifies `key` with the woocraft domain unless it already carries it.
...
/// Normalizes a locale identifier to a woocraft-supported code when possible.
///
/// Trims casing, `_` separators, and encoding/orthography suffixes
/// (`zh_CN.UTF-8`, `de@formal`); known Chinese/Japanese/English variants map
/// onto the supported set, unknown codes pass through unchanged.
...
/// Detects the locale from `LC_ALL`/`LANG` and activates it.
pub fn init() {
...
/// Activates a locale after normalization.
pub fn set_locale(locale: &str) {
  let locale = normalize_locale(locale);
  tracing::debug!(locale = %locale, "locale set");
  rust_i18n::set_locale(&locale);
}

/// Lists all locales: the supported set plus any registered custom locales.
...
/// Replaces the custom translation table for `locale`.
///
/// Keys inside the woocraft domain are filtered out; built-ins always win.
...
/// Merges `translations` into the custom table for `locale`.
///
/// Keys inside the woocraft domain are ignored.
...
/// Looks up a woocraft-domain key in `locale`, returning `None` when absent.
...
/// Looks up a woocraft-domain key in the active locale.
pub fn try_translate_woocraft(key: impl AsRef<str>) -> Option<String> {
  let locale = locale();
  try_translate_woocraft_in_locale(&*locale, key)
}

/// Translates a woocraft-domain key in `locale`, falling back to the key.
...
/// Translates a woocraft-domain key in the active locale.
pub fn translate_woocraft(key: impl AsRef<str>) -> String {
  let locale = locale();
  translate_woocraft_in_locale(&*locale, key)
}

/// Looks up `key` in `locale` across custom and built-in tables.
...
/// Looks up `key` in the active locale across custom and built-in tables.
pub fn try_translate(key: impl AsRef<str>) -> Option<String> {
  let locale = locale();
  try_translate_in_locale(&*locale, key)
}

/// Translates `key` in `locale`, falling back to the key itself.
...
/// Returns the human-readable display name of `locale`.
///
/// Resolution order: the built-in `i18n.name` translation, a custom
/// `i18n.name`, the locale code when custom translations exist for it, and
/// finally the raw locale code.
...
mod tests {
  ...
  #[test]
  fn incomplete_custom_translation_merges_with_builtin() {
  ...
  #[test]
  fn custom_translation_priority_over_builtin() {
    let _guard = LocaleTestGuard::new();
  ...
  #[test]
  fn custom_locale_cannot_override_woocraft_domain_key() {
    let _guard = LocaleTestGuard::new();
  ...
  #[test]
  fn extend_locale_preserves_builtin_translations() {
    let _guard = LocaleTestGuard::new();

    // Extend with just a few keys
    let mut partial_translations = HashMap::new();
  ...
  #[test]
  fn normalize_maps_family_variants_onto_supported_locales() {
    assert_eq!(normalize_locale("zh_CN.UTF-8"), "zh-hans");
    assert_eq!(normalize_locale("zh-TW@posix"), "zh-hant");
  ...
  #[test]
  fn woocraft_key_is_idempotent_over_the_domain() {
    assert_eq!(
      woocraft_key("common.loading"),
      format!("{WOOCRAFT_I18N_DOMAIN}.common.loading")
    );
    assert_eq!(
      woocraft_key(woocraft_key("common.loading")),
      woocraft_key("common.loading")
    );
  }

  #[test]
  fn built_in_locales_translate_and_display() {
    assert_eq!(
      translate_woocraft_in_locale("zh-hans", "common.loading"),
      try_translate_woocraft_in_locale("zh-hans", "common.loading")
        .expect("zh-hans should translate the loading key")
    );

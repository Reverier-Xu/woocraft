...
/// Syntax token hues used by editor syntax coloring.
///
...
impl Default for SyntaxTokenHues {
  fn default() -> Self {
    // Derived from the Catppuccin Mocha syntax palette. Neutral source-theme
    // tokens are remapped to nearby accent hues because Woocraft stores syntax
    // colors as hue-only accents.
    Self {
      attribute: 86.5,
      boolean: 52.6,
      comment: 277.3,
      ...
      property: 182.7,
      punctuation: 277.3,
      punctuation_bracket: 277.3,
      punctuation_delimiter: 182.7,
      punctuation_list_marker: 182.7,
      punctuation_special: 30.5,
      string: 142.7,
      string_escape: 336.3,
      string_regex: 336.3,
      string_special: 2.8,
      string_special_symbol: 18.0,
      tag: 259.9,
      tag_doctype: 304.8,
      text_literal: 142.7,
      title: 2.8,
      type_: 86.5,
      variable: 8.8,
      variable_special: 2.8,
      variant: 182.7,
    }
  }
}

/// oklch design tokens of the woocraft color system.
///
/// the design language is defined in oklch space: semantic hues
...
mod tests {
  ...
  #[test]
  fn unreadable_token_file_reports_path() {
    let error = ThemeTokens::from_toml_file("/nonexistent/woocraft-tokens.toml")
      .expect_err("missing file should fail");

    assert!(
      error
        .to_string()
        .contains("/nonexistent/woocraft-tokens.toml"),
      "error should name the unreadable path: {error}"
    );
  }

  #[test]
  fn malformed_tokens_report_format() {
    let error = ThemeTokens::from_json_str("{ not json }").expect_err("malformed json should fail");
    assert!(error.to_string().contains("json"), "error: {error}");

    let error =
      ThemeTokens::from_toml_str("primary = ). No.").expect_err("malformed toml should fail");
    assert!(error.to_string().contains("toml"), "error: {error}");
  }
}

...
/// TileItem is a moveable and resizable panel that can be added to a Tiles
/// view.
#[derive(Clone)]
pub struct TileItem {
  id: EntityId,
...
/// Tiles is a canvas that can contain multiple panels, each of which can be
/// dragged and resized.
pub struct Tiles {
...
impl Tiles {
  ...
  pub fn set_scrollbar_show(
    &mut self, scrollbar_show: Option<ScrollbarShow>, cx: &mut Context<Self>,
  ) {
  ...
  fn calculate_magnetic_snap(
    &self, dragging_bounds: Bounds<Pixels>, item_ix: usize, snap_threshold: Pixels,
  ...
  pub fn add_item(
    &mut self, item: TileItem, dock_area: &WeakEntity<DockArea>, window: &mut Window,
    cx: &mut Context<Self>,
  ) {
  ...
  /// Bring the panel of target_index to front, returns (old_index, new_index)
  /// if successful
  fn bring_to_front(
    &mut self, target_id: Option<EntityId>, cx: &mut Context<Self>,
  ) -> Option<EntityId> {

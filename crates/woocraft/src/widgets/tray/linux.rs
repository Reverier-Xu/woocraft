...
use super::{
  Tray, TrayEvent as RuntimeTrayEvent, TrayMenuItem, TrayMouseButton,
...
impl WorkerState {
  ...
  fn ensure_service(&mut self) -> Result<()> {
    ...
    .map_err(|err| Error::Backend(BackendError::platform("DbusService::new", err.to_string())))?;
  ...
  fn rebuild_menu(&mut self, tray: &Tray) -> Result<u32> {
    ...
    debug!(
      "linux menu rebuild: ids={}, revision={}",
      menu_ids.len(),
      revision
    );
...
pub(crate) fn create() -> Result<(Box<dyn PlatformTray>, Receiver<RuntimeTrayEvent>)> {
  ...
  Ok((Box::new(LinuxBackend { command_tx }), event_rx))
}
...
fn handle_tray_event(
  state: &WorkerState, event: TrayEvent,
  runtime_event_tx: &crossbeam_channel::Sender<RuntimeTrayEvent>,
) {
  match event {

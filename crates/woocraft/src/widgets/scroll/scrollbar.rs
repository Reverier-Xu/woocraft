...
impl ScrollbarStateInner {
  ...
  fn with_last_scroll(
    &self, last_scroll_offset: Point<Pixels>, last_scroll_time: Option<Instant>,
  ) -> Self {
    let mut state = *self;
  ...
  fn is_scrollbar_visible(&self) -> bool {
    ...
    if let Some(last_time) = self.last_scroll_time {
      Instant::now().duration_since(last_time).as_secs_f32() < FADE_OUT_DURATION
    } else {
      false
    }
  }
...
impl Element for Scrollbar {
  ...
  fn prepaint(
    &mut self, _: Option<&GlobalElementId>, _: Option<&InspectorElementId>, bounds: Bounds<Pixels>,
  ...
  fn paint(
    ...
    _: &mut Self::RequestLayoutState, prepaint: &mut Self::PrepaintState, window: &mut Window,
    cx: &mut App,
  ) {

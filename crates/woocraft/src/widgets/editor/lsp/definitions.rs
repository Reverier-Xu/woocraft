...
impl HoverDefinition {
  pub(crate) fn update(
    &mut self, symbol_range: Range<usize>, locations: Vec<lsp_types::LocationLink>,
  ) {
    self.clear();
...
impl InputState {
  ...
  pub(crate) fn on_action_go_to_definition(
    &mut self, _: &GoToDefinition, _: &mut Window, cx: &mut Context<Self>,
  ) {
  ...
  pub(crate) fn handle_click_hover_definition(
    &mut self, event: &MouseDownEvent, offset: usize, _: &mut Window, cx: &mut Context<InputState>,
  ) -> bool {
  ...
  pub(crate) fn go_to_definition(
    &mut self, location: &lsp_types::LocationLink, cx: &mut Context<Self>,
  ) {
    if location
      .target_uri
      .scheme()
...
impl ViewportElement {
  pub(crate) fn layout_hover_definition(&self, cx: &App) -> Option<(Range<usize>, HighlightStyle)> {
    ...
    let mut highlight_style: HighlightStyle = highlight_theme
      .link_text
      .map(Into::into)
      .unwrap_or_default();
  ...
  pub(crate) fn layout_hover_definition_hitbox(
    &self, editor: &InputState, window: &mut Window, _cx: &App,
  ) -> Option<Hitbox> {
    if !editor.mode.is_code_editor() {
      return None;
    }

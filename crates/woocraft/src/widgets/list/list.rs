  ...
  D: ListDelegate,
{
  ...
  /// Sets whether the list is searchable, default `false`.
  ...
  /// Sets whether the list is selectable, default `true`.
  ...
  /// Set index of right-clicked item.
  ...
  pub fn scroll_to_item(
    &mut self, ix: IndexPath, strategy: ScrollStrategy, _: &mut Window, cx: &mut Context<Self>,
  ) {
  ...
  /// Dispatch delegate `load_more` when visible range is near end.
  fn load_more_if_need(
    &mut self, entities_count: usize, visible_end: usize, window: &mut Window,
    cx: &mut Context<Self>,
  ) {
  ...
  pub(crate) fn on_action_select_prev(
    &mut self, _: &SelectUp, window: &mut Window, cx: &mut Context<Self>,
  ) {
    if self.rows_cache.len() == 0 {
      return;
    }
  ...
  pub(crate) fn on_action_select_next(
    &mut self, _: &SelectDown, window: &mut Window, cx: &mut Context<Self>,
  ) {
    if self.rows_cache.len() == 0 {
      return;
    }
  ...
  fn render_list_item(
    &mut self, ix: IndexPath, window: &mut Window, cx: &mut Context<Self>,
  ) -> impl IntoElement {
  ...
  D: ListDelegate + 'static,
{
  ...
  /// Set whether scrollbar is visible, default `true`.

...
use crate::widgets::editor::{InputState, Lsp, RopeExt};
...
pub trait DocumentColorProvider {
  ...
  fn document_colors(
    &self, _text: &Rope, window: &mut Window, cx: &mut App,
...
impl Lsp {
  ...
  pub(crate) fn document_colors_for_range(
    &self, text: &Rope, visible_range: &Range<usize>,
  ...
  pub(crate) fn update_document_colors(
    &mut self, text: &Rope, window: &mut Window, cx: &mut Context<InputState>,
  ) {

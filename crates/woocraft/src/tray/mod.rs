...
//!   maps `TrayEvent::MenuClicked { id }` to its own behaviors;
...
pub(crate) fn create_platform_tray() -> Result<(
  Box<dyn platform::PlatformTray>,
  crossbeam_channel::Receiver<TrayEvent>,
)> {
  #[cfg(target_os = "linux")]
  {
    linux::create()

...
impl ListDelegate for MenuDelegate {
  ...
  fn render_item(
    &mut self, ix: crate::IndexPath, _: &mut Window, _: &mut Context<ListState<Self>>,
  ) -> Option<Self::Item> {
  ...
  fn set_selected_index(
    &mut self, ix: Option<crate::IndexPath>, _: &mut Window, cx: &mut Context<ListState<Self>>,
  ) {
...
impl CodeActionMenu {
  ...
  pub(crate) fn handle_action(
    &mut self, action: Box<dyn Action>, window: &mut Window, cx: &mut Context<Self>,
  ) -> bool {
    if !self.open {
      return false;
    }
  ...
  pub(crate) fn show(
    &mut self, offset: usize, items: impl Into<Vec<CodeActionItem>>, window: &mut Window,
    cx: &mut Context<Self>,
  ) {
  ...
  fn origin(&self, cx: &App) -> Option<Point<Pixels>> {
    ...
    Some(
      cursor_origin - state.input_bounds.origin

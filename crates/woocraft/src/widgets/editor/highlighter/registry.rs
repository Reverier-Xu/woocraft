...
use crate::{
  ...
  widgets::editor::highlighter::{Language, languages},
};
...
impl LanguageConfig {
  pub fn new(
    name: impl Into<SharedString>, language: tree_sitter::Language,
    injection_languages: Vec<SharedString>, highlights: &str, injections: &str, locals: &str,
  ) -> Self {
    Self {
      name: name.into(),
      language,
...
impl SyntaxColors {
  pub fn from_tokens(tokens: &ThemeTokens) -> Self {
    let hues = tokens.syntax;
    Self {
      attribute: Some(ThemeStyle::with_color(tokens.syntax_color(hues.attribute))),
      boolean: Some(ThemeStyle::with_color(tokens.syntax_color(hues.boolean))),
      comment: Some(ThemeStyle::with_color(tokens.syntax_color(hues.comment))),
      comment_doc: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.comment_doc),
      )),
      constant: Some(ThemeStyle::with_color(tokens.syntax_color(hues.constant))),
      constructor: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.constructor),
      )),
      embedded: Some(ThemeStyle::with_color(tokens.syntax_color(hues.embedded))),
      emphasis: Some(ThemeStyle::with_color(tokens.syntax_color(hues.emphasis))),
      emphasis_strong: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.emphasis_strong),
      )),
      enum_: Some(ThemeStyle::with_color(tokens.syntax_color(hues.enum_))),
      function: Some(ThemeStyle::with_color(tokens.syntax_color(hues.function))),
      hint: Some(ThemeStyle::with_color(tokens.syntax_color(hues.hint))),
      keyword: Some(ThemeStyle::with_color(tokens.syntax_color(hues.keyword))),
      label: Some(ThemeStyle::with_color(tokens.syntax_color(hues.label))),
      link_text: Some(ThemeStyle::with_color(tokens.syntax_color(hues.link_text))),
      link_uri: Some(ThemeStyle::with_color(tokens.syntax_color(hues.link_uri))),
      number: Some(ThemeStyle::with_color(tokens.syntax_color(hues.number))),
      operator: Some(ThemeStyle::with_color(tokens.syntax_color(hues.operator))),
      predictive: Some(ThemeStyle::with_color(tokens.syntax_color(hues.predictive))),
      preproc: Some(ThemeStyle::with_color(tokens.syntax_color(hues.preproc))),
      primary: Some(ThemeStyle::with_color(tokens.syntax_color(hues.primary))),
      property: Some(ThemeStyle::with_color(tokens.syntax_color(hues.property))),
      punctuation: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.punctuation),
      )),
      punctuation_bracket: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.punctuation_bracket),
      )),
      punctuation_delimiter: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.punctuation_delimiter),
      )),
      punctuation_list_marker: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.punctuation_list_marker),
      )),
      punctuation_special: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.punctuation_special),
      )),
      string: Some(ThemeStyle::with_color(tokens.syntax_color(hues.string))),
      string_escape: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.string_escape),
      )),
      string_regex: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.string_regex),
      )),
      string_special: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.string_special),
      )),
      string_special_symbol: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.string_special_symbol),
      )),
      tag: Some(ThemeStyle::with_color(tokens.syntax_color(hues.tag))),
      tag_doctype: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.tag_doctype),
      )),
      text_literal: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.text_literal),
      )),
      title: Some(ThemeStyle::with_color(tokens.syntax_color(hues.title))),
      type_: Some(ThemeStyle::with_color(tokens.syntax_color(hues.type_))),
      variable: Some(ThemeStyle::with_color(tokens.syntax_color(hues.variable))),
      variable_special: Some(ThemeStyle::with_color(
        tokens.syntax_color(hues.variable_special),
      )),
      variant: Some(ThemeStyle::with_color(tokens.syntax_color(hues.variant))),
    }
  }
  ...
  fn from_tokens_with_mode(tokens: ThemeTokens, appearance: ThemeMode) -> Self {
    ...
    Self {
      name: if appearance.is_dark() {
        "Woocraft Dark".to_string()
      } else {
        "Woocraft Light".to_string()
      },
      appearance,
      style: HighlightThemeStyle {
        editor_background: Some(colors.editor_background),
        editor_foreground: Some(colors.editor_foreground),
        editor_active_line: Some(colors.editor_active_line),
        editor_line_number: Some(colors.editor_line_number),
        editor_active_line_number: Some(colors.editor_active_line_number),
        editor_invisible: Some(colors.editor_invisible),
        status: StatusColors::default(),
        syntax: SyntaxColors::from_tokens(&tokens),
      },
    }
  }

  pub fn from_theme(theme: &crate::Theme) -> Self {
    Self::from_tokens_with_mode(theme.tokens, theme.mode)
  }

  pub fn default_dark() -> Arc<Self> {
    Arc::new(Self::from_tokens_with_mode(
      ThemeTokens::default(),
      ThemeMode::Dark,
    ))
  }
    ...
    static INSTANCE: LazyLock<LanguageRegistry> = LazyLock::new(|| LanguageRegistry {
      languages: Mutex::new(
        languages::Language::all()
          .map(|language| {
            (
              SharedString::from(language.name().to_string()),
              language.config(),
            )
          })
          .collect(),

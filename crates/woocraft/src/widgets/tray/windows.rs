...
use super::{Tray, TrayEvent, TrayMenuItem, TrayMouseButton};
...
pub(crate) fn create() -> Result<(Box<dyn PlatformTray>, Receiver<TrayEvent>)> {
    ...
    Ok((
        Box::new(WindowsBackend { command_tx }),
        event_rx,
    ))
}
    ...
    match msg {
        ...
        WM_COMMAND => {
            ...
            if let Some(id) = state.menu_ids.get(&action_id) {
                let _ = state.event_tx.send(TrayEvent::MenuClicked { id: id.clone() });
            }
            return LRESULT(0);
        }
        _ => {
        ...
        match item {
            ...
            TrayMenuItem::Submenu { label, items } => {
                if let Some(sub) = build_menu(items, next_id, menu_ids) {
                    let wide = encode_wide(label.as_str());
                    let _ =
                        unsafe { AppendMenuW(menu, MF_POPUP, sub.0 as usize, PCWSTR(wide.as_ptr())) };
                }
            }
        }
    }

...
- **Default Type Scale**: all component text renders at the base size by inheriting the surrounding text style — components never apply the named size primitives (`text_xs`, `text_sm`, `text_lg`, …). those primitives keep their own scaling semantics for application-layer use; inside the library the only acceptable font size is the base one, and any exception must be a named, documented component like the reserved `dot-number`.
